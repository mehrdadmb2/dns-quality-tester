...
def aggregate(raw_results):
    ...
    sorted_summary = sorted(summaries, key=lambda x: x["composite_score"], reverse=True)
    return sorted_summary, majority_answer

def save_outputs(raw_results, summary, csv_path, json_path):
    # save raw CSV
    csv_fields = ["resolver", "resolver_ip", "domain", "qtype", "protocol", "success",
                  "rtt_ms", "answers", "flags", "dnssec_ad", "doh_present",
                  "doh_success", "doh_rtt_ms", "doh_answers", "doh_flags", "error", "attempt"]
...
def main():
    parser = argparse.ArgumentParser(description="DNS Quality Tester - PRO")
    parser.add_argument("--attempts", type=int, default=DEFAULT_ATTEMPTS, help="attempts per domain/qtype per resolver")
    parser.add_argument("--timeout", type=float, default=DEFAULT_TIMEOUT, help="UDP/TCP timeout (s)")
    parser.add_argument("--doh-timeout", type=float, default=10.0, help="DoH request timeout (s)")
    parser.add_argument("--workers", type=int, default=DEFAULT_WORKERS, help="concurrent worker threads")
    parser.add_argument("--spread", type=float, default=DEFAULT_SPREAD, help="spread total jitter seconds per task (0 = no spread)")
    parser.add_argument("--out", type=str, default=CSV_OUT_DEFAULT, help="CSV output file")
    parser.add_argument("--summary", type=str, default=JSON_SUMMARY_DEFAULT, help="JSON summary file")
    parser.add_argument("--resolvers", type=str, default=None, help="optional resolvers.json file to load extra resolvers")
    parser.add_argument("--domains-file", type=str, default=None, help="optional domains file (one per line)")
    parser.add_argument("--qtypes", type=str, default=",".join(DEFAULT_QTYPES), help="comma-separated qtypes to test, e.g. A,AAAA,MX")
    args = parser.parse_args()

    # load resolvers
    resolvers = DEFAULT_RESOLVERS.copy()
    if args.resolvers:
        user_res = load_resolvers_from_file(args.resolvers)
        if user_res:
            resolvers.update(user_res)
            print(f"Loaded {len(user_res)} resolvers from {args.resolvers}")

    # load domains
    domains = DEFAULT_TEST_DOMAINS.copy()
    if args.domains_file:
        try:
            ...
            print("Failed to read domains file:", e)

    qtypes = [q.strip().upper() for q in args.qtypes.split(",") if q.strip()]

    print("Configuration:")
    print(f" resolvers: {len(resolvers)}")
    print(f" domains: {len(domains)}")
    print(f" qtypes: {qtypes}")
    print(f" attempts per pair: {args.attempts}")
    print(f" timeout: {args.timeout}s, doh-timeout: {args.doh_timeout}s")
    print(f" workers: {args.workers}, spread jitter: {args.spread}s")
    print("Starting tests... (this may take a while depending on attempts & spread)")

    raw = run_all_tests(resolvers, domains, qtypes, args.attempts, args.timeout, args.workers, args.spread, args.doh_timeout)
    print(f"Collected {len(raw)} samples. Aggregating...")

    summary, majority_map = aggregate(raw)

    # print top 8 resolvers
    print("\nTop resolvers by composite score:")
    for i, s in enumerate(summary[:8], 1):
        print(f"{i}. {s['resolver']} ({s['ip']}) — score {s['composite_score']}, avg_rtt={s['avg_rtt_ms']} ms, success={s['success_rate']}, dnssec={s['dnssec_rate']}, disagree={s['disagreement_rate']}")

    # save files
    csv_path = args.out
    json_path = args.summary
    save_outputs(raw, summary, csv_path, json_path)
    print("\nDone. Inspect CSV for raw per-request details and JSON summary for ranks.")
